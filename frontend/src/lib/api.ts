...
function computeMetadata(text: string): {
  wordCount: number;
  characters: number;
  size: string;
} {
...
export async function fetchAdminEssays(): Promise<Document[]> {
  ...
  const documents: Document[] = Object.entries(data).map(
    ([title, versions]: [string, any[]], index) => {
      ...
      return {
        ...
        lastModified:
          parsedVersions.at(-1)?.timestamp || parsedVersions[0].timestamp,
